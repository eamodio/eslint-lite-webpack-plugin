...
const match =
	/\[unreleased\]: https:\/\/github\.com\/eamodio\/eslint-lite-webpack-plugin\/compare\/v(.+)\.\.\.HEAD/.exec(data);
...
rl.question(`Enter the new version number (format x.x.x, current is ${currentVersion}): `, function (version) {
	...
	const newVersionLink = `[${version}]: https://github.com/eamodio/eslint-lite-webpack-plugin/compare/v${currentVersion}...eamodio:v${version}`;
